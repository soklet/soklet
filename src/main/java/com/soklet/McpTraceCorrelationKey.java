...
/**
 * Immutable secret key used only for pseudonymous MCP trace correlation.
 * <p>
...
public final class McpTraceCorrelationKey {
	...
	/**
	 * Creates a trace-correlation key from exact bytes.
	 *
	 ...
	 */
	@NonNull
	public static McpTraceCorrelationKey fromIdAndBytes(@NonNull String keyId,
			byte @NonNull [] keyMaterial) {
		return new McpTraceCorrelationKey(keyId, keyMaterial);
	}

	private McpTraceCorrelationKey(@NonNull String keyId,
			byte @NonNull [] keyMaterial) {
		this.keyId = McpKeyIdValidator.validate(keyId,
				"MCP trace-correlation key ID");
		requireNonNull(keyMaterial);
		if (keyMaterial.length < MINIMUM_KEY_BYTES)
			throw new IllegalArgumentException(
					"MCP trace-correlation keys must contain at least 32 bytes.");
