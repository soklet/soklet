...
const metadata = {[`${META}serverInfo`]: {name: 'inspector-auth-patch-regression', version: 'fixture-v1'}};
const challenges = Object.freeze({
  'auth-401-invalid-token': Object.freeze({status: 401, reason: 'invalid_token',
    header: 'Bearer error="invalid_token"'}),
  'auth-403-insufficient-scope': Object.freeze({status: 403, reason: 'insufficient_scope',
    header: 'Bearer error="insufficient_scope", scope="catalog.read"'}),
});
...
// This dependency-free responder isolates a host transport patch. It does not
// imitate a complete MCP server or qualify Soklet. In particular, returning an
// error with HTTP 200 is a causal control, never a proposed server fix. Explicit
// Bearer challenges provide positive controls, with every recovery request still
// refused on the same loopback listener; no OAuth service is contacted.
...
export async function startFixture({token, mode} = {}) {
  ...
  const server = createServer(async (incoming, outgoing) => {
    ...
    try {
      ...
      const challenge = subscriptionDenied ? challenges[mode] : undefined;
      const responseStatus = challenge?.status ?? (subscriptionDenied && mode === 'policy-403' ? 403 : 200);
      // Only fixed labels, booleans and bounded counts leave this fixture.
      rows.push({surface: 'inspector-auth-patch', sequence: rows.length + 1, requestSequence,
        ...
        authChallenge: challenge?.reason ?? 'none', wwwAuthenticatePresent: challenge !== undefined,
        requiredScopeCatalogRead: challenge?.reason === 'insufficient_scope',
      ...
      outgoing.writeHead(responseStatus, {'content-type': 'application/json', 'cache-control': 'no-store',
        ...(challenge ? {'www-authenticate': challenge.header} : {})});
