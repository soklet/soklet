...
function sourceInputs() {
  ...
  visit(resolve(here, '../apps-disabled-host'));
  visit(resolve(here, '../apps-mime-boundary'));
...
export async function runHost(options) {
  ...
  try {
    ...
    while (Date.now() < observeUntil) {
      if (interrupted || hostExited || browserExited || cdp.failure() || proxy.failure() || observation.failure() || canary.failure())
    ...
    receipt.failure = CSP_FAILURES.includes(error?.message) || /^APPS_(?:HOST|BROWSER|UI)_[A-Z_]+$/.test(error?.message)
      ? error.message : 'APPS_HOST_PROBE_FAILED';
    receipt.traceFailure = proxy?.failure() ?? null;
    receipt.browserFailure = observation?.failure() ?? cdp?.failure() ?? null;
    receipt.canaryFailure = canary?.failure() ?? null;
  } finally {
    let cdpClosed = true;
    ...
    receipt.cleanup = {cdp: cdpClosed, browser: cleanup[0].status === 'fulfilled', host: cleanup[1].status === 'fulfilled',
      proxy: cleanup[2].status === 'fulfilled', fixture: cleanup[3].status === 'fulfilled', canary: canaryCleanup};
    receipt.canary = canary?.facts() ?? null;
    receipt.canaryFailure = canary?.failure() ?? null;
    receipt.csp ??= observation?.facts().cspEvidence ?? null;
    ...
    if (!cdpClosed || !canaryCleanup || cleanup.some(row => row.status !== 'fulfilled')) {
    ...
    // Drain an interruption queued during the synchronous identity checks.
