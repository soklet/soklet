...
export function parseArguments(args) {
  const keys = ['--dependencies','--browser','--work-dir'];
...
export function adjudicateCase(row) {
  ...
  if (!profile || row.mode !== profile.mode || row.fixtureFailure || row.failure
      ...
      || !Number.isSafeInteger(row.browserExceptions) || row.browserExceptions < 0
  ...
  if (calls.length < 3 || calls.length > 32 || calls[0]?.method !== 'server/discover'
      ...
      || calls.some((call,i) => call?.sequence !== i+1 || call.surface !== 'inspector-auth'
        ...
        || call.responseStatus !== (call.method === 'subscriptions/listen' && profile.mode === 'policy-403' ? 403 : 200)
...
export function adjudicateMatrix(receipt) {
  if (receipt.failure || receipt.inputsUnchanged !== true || receipt.runs?.length !== CASES.length) return 'FAILED';
  return receipt.runs.every((row,i) => row.name === CASES[i].name
    && adjudicateCase(row) === (CASES[i].mode === 'policy-403' ? 'BUG_REPRODUCED' : 'CONTROL_PASSED'))
    ? 'REPRODUCED_WITH_CONTROLS' : 'FAILED';
}
...
function sourceInputs() {
  ...
  visit(here); visit(resolve(here,'../inspector'));
...
async function probe({profile,work,browserPath,entry}) {
  const privateRoot=resolve(work,'private-'+profile.name);
  ...
  const row={name:profile.name,mode:profile.mode,status:'FAILED',stage:'SETUP',
...
export async function run(options) {
  ...
  const dependencies=resolve(options['--dependencies']),modules=resolve(dependencies,'node_modules');
  const inspector=resolve(modules,'@modelcontextprotocol/inspector');
  const entry=regular(resolve(inspector,'clients/launcher/build/index.js'));
  ...
  if(JSON.stringify(directoryIdentity(modules))!==JSON.stringify(installedPin))fail('AUTH_INSTALLED_DRIFT');
  const work=resolve(options['--work-dir']);
  if(existsSync(work)||(work.startsWith(root+'/')&&!work.startsWith(resolve(root,'target')+'/')))fail('AUTH_WORK_DIRECTORY');
  ...
  const receipt={formatVersion:1,profile:'soklet.inspector.policy-denial-auth-recovery.v1',
    ...
    installedTreeIdentity:installedPin,browserDistributionIdentity:directoryIdentity(browserRoot),inputs:sourceInputs(),
    ...
    limitations:['A minimal diagnostic fixture, not Soklet conformance or Apps host qualification.',
      ...
      'A reproduction success records a host defect, not a passing host.'],runs:[]};
  let interrupted=false;
  const signal=()=>{interrupted=true;receipt.failure='AUTH_INTERRUPTED';};
  process.on('SIGINT',signal);process.on('SIGTERM',signal);
  try {
    await collectCases(profile=>probe({profile,work,browserPath,entry}), row=>{
    ...
    try {
      ...
      receipt.inputsUnchanged=JSON.stringify(sourceInputs())===JSON.stringify(receipt.inputs)
        &&JSON.stringify(directoryIdentity(modules))===JSON.stringify(installedPin)
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(json({status:receipt.status,hostQualification:false,runs:receipt.runs.map(({name,status,failure})=>({name,status,failure}))}).trim());
    if(receipt.status!=='REPRODUCED_WITH_CONTROLS')process.exitCode=1;
  }catch{console.error('Inspector auth regression rejected inputs; no qualified receipt.');process.exitCode=1;}
}
