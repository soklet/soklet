...
function verifyCharacterizationSources(root, sourceFiles) {
  ...
  requireStructural(withPort, /return\s+new\s+Builder\s*\(\s*requirePort\s*\(\s*requireNonNull\s*\(\s*port\s*\)\s*\)\s*\)\s*;/u, 'McpServer.withPort must prime its dedicated builder');
