...
dependencies {
  implementation 'com.soklet:soklet:2.1.2-SNAPSHOT'
}
```

#### Direct Download

If you don't use Maven or Gradle, you can drop [soklet-2.1.2-SNAPSHOT.jar](https://repo1.maven.org/maven2/com/soklet/soklet/2.1.2-SNAPSHOT/soklet-2.1.2-SNAPSHOT.jar) directly into your project. No other dependencies are required.

### Code Sample

Here we demonstrate building and running a single-file Soklet application with nothing but the [soklet-2.1.2-SNAPSHOT.jar](https://repo1.maven.org/maven2/com/soklet/soklet/2.1.2-SNAPSHOT/soklet-2.1.2-SNAPSHOT.jar) and the JDK. There are no other libraries or frameworks, no Servlet container, no Maven or Gradle build process - no special setup is required.
...
```shell
javac -parameters -cp soklet-2.1.2-SNAPSHOT.jar -processor com.soklet.SokletProcessor -d build src/com/soklet/example/App.java
```

#### Run

```shell
java -cp soklet-2.1.2-SNAPSHOT.jar:build com/soklet/example/App
```
